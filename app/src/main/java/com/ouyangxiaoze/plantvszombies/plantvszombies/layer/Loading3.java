 ...
 *
 */
public class Loading3 extends BaseLayer {
	...
	private void setLoadingAnimation() {
		...
		loadingAnimation = CCAnimation.animation("",1f,loadFreams);
...
//				progressLoad.getAction(1).i
				CCSpriteFrame displayedFrame = CCSprite.sprite("image/loading/loading_09.png").displayedFrame();
				if(progressLoad.isFrameDisplayed(displayedFrame))
				{
					start();
				}else
				{
					CCSpeed speed=CCSpeed.action(loadingAnimate, 6);
					progressLoad.runAction(speed);
//					progressLoad.runAction(CCSequence.actions(speed, CCCallFunc.action(this, "start")));
				}
				if (result) {
					// TODO 提示用户是否进行版本更新
				}
	...
	private void start() {
		progressLoad.setDisplayFrame(CCSprite.sprite("loading_start.png").displayedFrame());
		isClick = true;
	};
